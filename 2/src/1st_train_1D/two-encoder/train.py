...
def main():
    ...
    for fold in range(3):
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1.e-3)
        early_stopping = Earlystopping(patience=10, verbose=True, model_name=f'model/model{fold}.pth')
        criterion =  nn.BCEWithLogitsLoss()
        scheduler = CosineLRScheduler(optimizer, t_initial=100, lr_min=1.e-5, warmup_t=10, warmup_lr_init=1.e-5, warmup_prefix=True)
